...
//			}
			...
			for (int i = periodsOfItem.nextSetBit(0); i >= 0; 
				...
				if(isPromisingInAtLeastOnePeriod == false) {
					if(pair.estimatedUtility[i] / ((double) periodUtility) >= minUtilityRatio) {
...
//			// IF IT IS NOT A CANDIDATE THEN WE WILL NOT CONSIDER THIS ITEM ANYMORE
			...
			}else if(pair.exactUtility / ((double) sumPeriodUtility) >= minUtilityRatio) {
...
//				}
					...
					if(estimatedUtilityIJ[i] != null) {
						...
						if(isPromisingInAtLeastOnePeriod == false) {
							if(estimatedUtilityIJ[i] 
									/ ((double) periodUtilities.get(i)) >= minUtilityRatio) {
								isPromisingInAtLeastOnePeriod = true;
							}
						}
				...
				if(isPromisingInAtLeastOnePeriod == false) {
					...
					if(exactUtilityIJ / ((double)sumOfPeriodUtility) >= minUtilityRatio) {
...
//				// for each period containing the itemset
	loop:		for (int period = periods.nextSetBit(0); period >= 0; 
				...
				if(exactUtility / ((double)sumPeriodUtility) >= minUtilityRatio) {
...
//				if(compare(itemset, trans.getItemsUtilities()) > 0) {
			...
			if(estimatedUtility / ((double)periodUtility) < minUtilityRatio) {
				iter.remove();
			}
//			// else we keep it
			if(exactUtility / ((double)periodUtility) >= minUtilityRatio) {
